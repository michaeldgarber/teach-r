...
For additional details on package management, please see this module: [https://michaeldgarber.github.io/teach-r/manage-packages.html](https://michaeldgarber.github.io/teach-r/0-managing-packages.html){.uri}
...
**Side-note**: the `package_name::object` syntax, as in `datasets::LakeHuron`, is R's way of explicitly stating which package the object is coming from. Technically, it loads the package via namespace, elaborated upon here: <https://michaeldgarber.github.io/teach-r/0-manage-packages.html>. If the package has been attached via `library(package_name)`, then we could write, `lake_huron_raw = LakeHuron`.
