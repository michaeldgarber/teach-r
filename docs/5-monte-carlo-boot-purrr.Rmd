...
Take note the unusual structure of this code. The function we created above, `monte_carlo_sim_fun` is inside `map_dfr()`, and the list of replication ids, `rep_id_val_list,` is the object that begins the pipe sequence.
...
What's the distribution of `n_cases_mean` over Monte Carlo replications? This is not the variability of the number of cases within the state between counties but rather the variability of the mean number of cases over counties between replications.
...
In addition to their use for simulations as above, functions can also be used simply to reduce repetition in code. Sometimes you end up writing the same or similar code many times and apply that code to several datasets. For example, above we wrote an almost identical equence of dplyr functions to define both `monte_carlo_sim_frozen` and the function, `monte_carlo_sim_fun()`.
