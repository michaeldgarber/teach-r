...
In addition to their use for simulations as above, functions can also be used simply to reduce repetition in code. Sometimes you end up writing the same or similar code many times and apply that code to several datasets. For example, above we wrote basically the same sequence of dplyr functions to define `monte_carlo_sim_frozen` as we did to define the function, `monte_carlo_sim_fun()`. Here is a set of dplyr steps combined into one function.
