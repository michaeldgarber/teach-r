...
In this demo, we introduce Poisson regression and go through an example using the British Doctor's Data.
...
-   y is the number of events (i.e., a count variable),

-   t is follow-up time (or generally the rate's denominator),

-   b0 is the intercept,

-   b1 is the coefficient for variable x1,

-   b2 is the cofficient for independent variable x2,
...
This ln(t) is what is known as the "log offset" when fitting the Poisson model for rate data in software.
...
The videos use the British doctor's data, a famous study on lung cancer and smoking: <https://en.wikipedia.org/wiki/British_Doctors_Study>
...
My intro data wrangling in the tidyverse is here: <https://michaeldgarber.github.io/teach-r/dplyr-1-nyt-covid.html>

The canonical source on the tidyverse is R for Data Science <https://r4ds.had.co.nz/wrangle-intro.html>
...
So the rate is `Deaths`/`Person_Years`.

# Fit a Poisson regression to model the rate

Following video 9.10 above, let's model the rate as a function of the smoking indicator variable (yes or no; `Smoke_Ind`) and the age category (`Age_Group`).

We use the `glm()` function to fit this model. Documentation is here: <https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/glm>
...
sThe rate ratio corresponding to each predictor can be found by exponentiating the coefficient. The coefficients can be accessed by the syntax `model_object$coefficients`
