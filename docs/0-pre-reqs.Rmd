---
title: "First things first: background and prereqs before dplyr"
...
[Definitions of statistics aside](https://www.yourdictionary.com/statistics), the point is that R is a programming language that allows you to perform most any data-manipulation, data-analysis, or data-visualization task (including statistics).
...
3.  It can manipulate spatial data and make maps. No need to switch software for GIS.
...
-   **Using a [script](https://rstudio-education.github.io/hopr/basics.html#scripts)**. I almost always write code using a [script](https://rstudio-education.github.io/hopr/basics.html#scripts), which is essentially a text file that can be saved and edited as you would a Word document.
...
The book by Grolemund provides an overview of [RStudio's user interface](https://rstudio-education.github.io/hopr/basics.html#the-r-user-interface).
...
2.  Typing file paths is prone to error.
...
(Jenny Bryan's [ode](https://github.com/jennybc/here_here) to the here package is worth a read.)
...
You don't have to use projects to use R or RStudio, but I think it's a good idea, in part because of how well projects pair with the **here** package.
...
And if we want to change it back to the original directory containing the project? Simply use `here()` without specifying anything.
...
lake_huron_raw #Take a look
```

I find that to be hard to read. Let's clean it up and save it.
