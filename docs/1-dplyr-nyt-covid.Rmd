...
An alternative, equivalent, way to write it would be to use the traditional nested parentheses. The nested parentheses aren't so bad when just a few functions are used, but as the number of functions increases, the nested parentheses become difficult to read.
...
Either way we write it, we get a resulting dataset, `nyt_state_data`, which should be visible in the top-right corner of RStudio's interface. It tells us that the data have 18,934 observations and 5 variables. (The number of observations may be higher when you run it.)

```{r, echo=FALSE}
library(here)
#note you can do this in a code chunk to use the here package
knitr::include_graphics(here("docs", "images", "nyt-state-data-image.png"))
```


As noted above, `read_csv()` is part of the **readr** package. Accordingly, its default behavior is to read in data as a "[tibble](https://r4ds.had.co.nz/tibbles.html)", tidyverse speak for "tidy table". One benefit of using tibbles, as noted [here](https://r4ds.had.co.nz/tibbles.html), is that they can be printed without overwhelming the console. (This is not necessarily true of data frames more generally).

Let's first confirm that `nyt_state_data` is a tibble using the base-R `class()` function, which returns the object's class. The tibble object class is denoted by `tbl_df`. And then we can print the data to get a sense of its dimensions (rows and columns) and what the variables look like.

At this writing, there are 22,509 observations and 5 columns. The variable type is denoted below the variable names. We can also use class to grab the type for a single column.

```{r explore NYT 1, eval=TRUE, echo=TRUE, warning=FALSE, message=TRUE}
class(nyt_state_data)
nyt_state_data
...
If the direct call to NYT GitHub didn't work, here's a back-up way that should work to read the data into R. On February 9th, 2021, I downloaded the data as a CSV. They're available in this Dropbox folder: <https://www.dropbox.com/sh/crauwqsy64r33k7/AAAGuyfNQlSzws3Ey-uj012fa?dl=0>. If needed, download the CSV, save locally to your computer, and then use `read_csv()` to [import](https://r4ds.had.co.nz/data-import.html) the csv into R. `setwd()` function used to set the working directory, and `file.path()` is a convenient way to make your file path to be [operating-system independent](https://stackoverflow.com/questions/51533664/file-path-function-in-r). `read_csv()` loads the data relative to the working directory. That is, if you assign your working directory to be `C:\hi\how\are`, but the data are in `C:\hi\how\are\you`, then in the `read_csv()` call, you'd write `("you\filename.csv")`.

```{r Set working directory, eval=FALSE, echo=TRUE, message=FALSE}
setwd(file.path("C:", "Users", "maybe your name", "a subfolder", "maybe another subfolder", "the folder with the csv"))
nyt_state_data = read_csv("nyt-covid-19-state-2021-02-09.csv")
```

### Explore the data a bit more before further manipulation using dplyr verbs.

Aside from printing the tibble by simply typing its name (`nyt_state_data`) we can use `View` (note the capital *V*) to view the data in a new window and interact with it in a more hands-on way. `View()` can be slow as the number of observations rises, but 20,000-ish shouldn't be too bad. `View()` allows you to sort the data by values in the columns. We can sort the data by state in the `View()` window to confirm that the data are, in fact, cumulative: cases and deaths only increase.
...
-   `arrange()` sorts data. It orders the rows of a data frame by the values of selected columns. The default behavior is to sort ascending, meaning the lowest value will be on top. Use `desc()` within the `arrange()` function to sort descending.
...
-   `summarise()` creates a new data frame, with rows corresponding to combinations of the grouping variables. `summarise()` is like a summary operator in an Excel pivot table.
-   `left_join()` joins two tables together. It's like a vlookup in Excel or, as the name implies, a JOIN in SQL. It's distinct from the other verbs in this list in that it is a [two-table verb](https://dplyr.tidyverse.org/articles/two-table.html). There are [other types of joins](https://dplyr.tidyverse.org/reference/join.html), but I almost exclusively use `left_join()`. In the below set of examples, `left_join` isn't used. It is used in our [next tutorial](https://rpubs.com/michaeldgarber/dplyr_2_tidycensus_mapview), though.
...
For a demo of `mutate()`, let's create a new variable called `cases_plus_1`, which is simply the number of cases plus 1. The first argument inside the `mutate()` call is the object to be operated on. And the second argument creates the new variable as a function of the old variable and, in this example, the number 1. Adding the number 1 to the *cases* variable works because *cases* is a numeric variable. If *cases* were a character variable, it would not work.

```{r mutate 1, eval=TRUE, echo=TRUE, warning=FALSE, message=TRUE}
nyt_state_data_2 = mutate(nyt_state_data, cases_plus_1 = cases+1)
```

Now let's try the same thing but written with the pipe (`%>%`) operator. As mentioned above, the pipe brings the object out of the parentheses to focus on the action-oriented part of the function, making it easier to read.
...
The number of observations decreased, as we'd expect. There are now 402 observations (at this writing).
...
date_pipe = nyt_state_data %>% dplyr::select(date)
```

Confirm that there's just one variable using `dim(date_pipe)`.
...
nyt_state_data_nodeaths = nyt_state_data %>% dplyr::select(everything(), -deaths)
...
`sum()`, `mean()`, and `max()` are arithmetic functions in base R. In these functions, it's good practice state `na.rm=TRUE` unless there's a good reason not to. This argument stands for "remove missings equals true". The default behavior of these arithmetic functions is to include missings (`na.rm = FALSE`), which means the arithmetic summary will also return a missing, probably not a desirable outcome.
...
Another application of the group-by-summarise syntax is if one wants to calculate overall summary estimates of the whole dataset. For example, as a double check, we can check to confirm the data we're using lines up with what the NYT is reporting on their website for the total number of cumulative cases to date. At this writing (4/15/21), NYT reports 31.4 million cases (<https://www.nytimes.com/interactive/2021/us/covid-cases.html>). Can we reproduce that? To summarize over the whole dataset, create a dummy variable with the same value for every observation and them sum over that variable.
...
Confirmed. The total cumulative number of cases at this writing is 31,444,696, which is about what they're reporting on their website today (31.4 million+). Note this total includes the cases from Guam, Northern Mariana Islands, Puerto Rico, and the Virgin Islands.

## All together now in a simple example using `case_when()`

Goal: calculate the total number of deaths in each state in the [Great Lakes Region](https://en.wikipedia.org/wiki/Great_Lakes_region): Illinois, Indiana, Michigan, Minnesota, New York, Ohio, Pennsylvania, and Wisconsin. Do it all in one sequential pipe. In this code, I'll introduce the `case_when()` function, which is a useful way to create new variables conditional on values of other variables.
...
deaths_cumul_by_state_greatlakes = nyt_state_wrangle %>% 
  ...
  #Filtering to states in the Great Lakes Region could be done a few different ways.
  #With complicated subsets, I prefer to create an indicator variable and then filter based on that indicator variable, 
  #rather than putting all of the subset information into the filter() function. 
  ...
  mutate(
    ...
    #A more concise way to state that may arguably be less readable is to use %in%, 
    #which is an operator used to assess whether an element belongs to a vector. 
    #It returns a TRUE or FALSE for each row.
...
We expect it to be a pretty small dataset (8x2), so let's print the whole thing

```{r great lakes check, eval=TRUE, echo=TRUE, warning=FALSE, message=TRUE}
deaths_cumul_by_state_greatlakes
```

That concludes this tutorial. In our next tutorial (<https://michaeldgarber.github.io/teach-r/2-dplyr-mapview-tidycensus.html>), we'll continue using dplyr and we'll introduce **tidycensus**, **mapview**, and **ggplot** to use census data to visualize incidence per population.
